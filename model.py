...
# Hybrid patch embedding with Conv2D
    ...
    def __init__(self, config):
        ...
        if self.pretraining:
            self.head = nn.Sequential(
                nn.Linear(config.n_embd, config.mlp_dim, bias=config.bias),
                nn.ReLU(),
                nn.Linear(config.mlp_dim, config.num_classes, bias=config.bias)
            )
        else:
            self.head = nn.Linear(config.n_embd, config.num_classes, bias=config.bias)
